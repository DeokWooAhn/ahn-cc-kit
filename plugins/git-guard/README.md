...
되돌리기 어려운 git 작업을 막는 훅 묶음. Android와 무관한 것만 모았습니다.
...
기본 보호 목록은 `main,master,release/*`입니다. glob이 먹습니다.
...
**브랜치를 가리지 않습니다.** 자기 feature 브랜치라도 `--force`는 무조건 덮어쓰므로, 마지막
...
- `git restore --staged .` — 인덱스만 되돌리므로 워킹트리는 안전합니다
- `git clean -nd` — dry run은 통과
- `git branch -d` — 병합 안 됐으면 git이 알아서 거절합니다
...
훅을 **`/bin/bash`로 돌립니다.** macOS 기본 셸은 아직 bash 3.2이고, 3.2는 `set -u` 아래에서
