...
cat >&2 <<'MSG'
Gradle 캐시를 지우지 않습니다. 재다운로드 비용이 크고, 원인이 데몬 쪽이면 지워도 그대로입니다.

순서대로 확인합니다.
  1. ./gradlew --stop    같은 Gradle 버전의 데몬만 멈춥니다. 다른 버전은 남습니다.
  2. jps                 모든 버전의 데몬을 봅니다. --status 는 현재 버전만 보여줍니다.
  3. IDE는 자기 버전으로 별도 데몬을 띄웁니다. wrapper의 --stop 이 닿지 않습니다.
  4. JAVA_HOME, toolchain, IDE의 JDK가 어긋나면 기존 데몬과 호환되지 않아 새로 뜹니다.

경로를 뒤지기 전에 echo $GRADLE_USER_HOME 부터 확인합니다. 기본값은 ~/.gradle 이지만
옮겨져 있을 수 있고, 빈 검색 결과를 "의존성이 해석된 적 없음"으로 읽으면 틀립니다.

정말로 지워야 한다면 사용자에게 재다운로드 비용을 알리고 확인받습니다.
MSG
exit 2
