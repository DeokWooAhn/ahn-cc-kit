...
MSG="⚠️ release 빌드를 돌렸는데 서명 설정을 찾지 못했다: $MISSING (환경 변수에도, local.properties에도 없음). 서명 키를 조건부로 구성하는 Gradle 패턴은 키가 없으면 signingConfig를 건너뛰고 빌드는 그대로 성공하므로, 산출물에 서명 블록이 없을 수 있다. 배포용이라면 CI에서 빌드하거나, apksigner verify <apk> / jarsigner -verify <aab> 로 확인한다. 이 프로젝트가 다른 이름의 키를 쓴다면 ANDROID_GUARD_SIGNING_KEYS로 맞춘다."
